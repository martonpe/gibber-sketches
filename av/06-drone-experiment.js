r= Reverb({wet2: .1}).bus().connect()
dl = Delay({time: 3/8}).bus().connect(r)
ds = Delay({time: 1/60, feedback: .9}).connect(dl)
td = Sampler('http://127.0.0.1:8080/teardrops.mp3', {loudness: .1})
...
f = FM[5]('bass', {decay: 3, attack: 1, gain: .2})
f.connect(ld, .4)
f.connect(r, 1)
...
osc(6,0.03,2).out(o1);
src(s0).out(o2);
src(o0).modulateHue(src(o0).scale(1.002).kaleid(10),100).blend(src(o2), ()=>1-sp.out(12)).blend(src(o1),.02).out()
